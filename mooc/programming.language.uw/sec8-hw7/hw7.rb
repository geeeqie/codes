...
class LineSegment < GeometryValue
  ...
  def intersectPoint p
      ...
      p_result = two_points_to_line(@x1,@y1,@x2,@y2).intersect(p)
      if p_result == p and inbetween(p.x,@x1,@x2) and inbetween(p.y,@y1,@y2)
          Point.new(p.x,p.y)
      else
          NoPoints.new
      end
  ...
  def intersectLine line
      line_result = two_points_to_line(@x1,@y1,@x2,@y2).intersect(line)
      if line_result == line
          self
      else
          intersectPoint(line_result)
      end
  end
  ...
  def intersectWithSegmentAsLineResult s
      ...
      if real_close(x1start,x1end)
          ...
          ((aXstart,aYstart,aXend,aYend), (bXstart,bYstart,bXend,bYend)) =
              (x1start < x2start)?  [seg,seg2] : [seg2,seg]
