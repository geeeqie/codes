...
(define tests
  (test-suite
   ...
   (check-equal? (list-nth-mod (list 1 2 3) 3) 1 "list-nth-mod test")
   
   ; stream-for-n-steps test
   (check-equal? (stream-for-n-steps (lambda () (cons 1 ones)) 1) (list 1) "stream-for-n-steps test")
   (check-equal? (stream-for-n-steps (lambda () (cons 1 ones)) 5) (list 1 1 1 1 1) "stream-for-n-steps test")
   ...
   (check-equal? (vector-assoc 1 (vector '(2, 20, 1) '(2) '(3) '(2))) #f "vector-assoc test")
   (check-equal? (vector-assoc 1 (vector '() #f '(2 '(1)) (cons 1 2))) (cons 1 2) "vector-assoc test")
