...
def findMaxExpenses(salary, save, preRetireGrowthRates, postRetireGrowthRates,
    ...
    savings = savingsRecord[len(preRetireGrowthRates) - 1]
    high = savings
    low = 0
    while True:
        expenses = (high+low) / 2
        print "expenses=" + str(expenses)
        postRec = postRetirement(savings, postRetireGrowthRates, expenses)
        postSav = postRec[len(postRetireGrowthRates) - 1]
        print "postSav=" + str(postSav)
        if abs(postSav) < epsilon:
            return expenses
        if postSav > 0:
            low = expenses
        else:
