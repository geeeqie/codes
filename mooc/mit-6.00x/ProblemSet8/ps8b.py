...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for tm in range(300):
            avg_pop[tm] += patient.update()/float(numTrials)
    print avg_pop

    pylab.plot(range(300), avg_pop)
    pylab.title("virus population variation without drug")
        ...
        if random.random() < self.maxBirthProb * (1 - popDensity):
            for resistance in self.resistances:
                if random.random() < self.mutProb:
                    self.resistances[resistance] = not self.resistances[resistance]
