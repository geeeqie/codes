...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(call? e) (let ([v1 (eval-under-env (call-funexp e) env)]
                     ...
                     (if (closure? v1)
                         (letrec ([f (closure-fun v1)]
                                  [env1 (cons (cons (fun-formal f) v2) env)]
                                  [env2 (let ([name (fun-nameopt f)])
                                             (if name
                                                 (cons (cons name v1) env1)
                                                 env1))])
                           (eval-under-env (fun-body f) env2))
                         (error "MUPL call parameter 1 not a closure")))]
        [(apair? e) (apair (eval-under-env (apair-e1 e) env)
                           (eval-under-env (apair-e2 e) env))]
        [(fst? e) (let ([v (eval-under-env (fst-e e) env)])
                    (if (apair? v)
                        (apair-e1 v)
                        (error "MUPL fst: not a pair")))]
        [(snd? e) (let ([v (eval-under-env (snd-e e) env)])
                    (if (apair? v)
                        (apair-e2 v)
                        (error "MUPL snd: not a pair")))]
...
;; I nearly work out! but time is over..
